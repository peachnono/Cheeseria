...
const CheeseRoutes = (cheeseService) => {
    ...
    router.get("/cheeses/:id/price", async (req, res) => {
        const cheeseId = parseInt(req.params.id, 10);
        try {
    ...
    router.get("/cheeses/:id/picture", async (req, res) => {
        const cheeseId = parseInt(req.params.id, 10);
        try {
    ...
    router.get("/cheeses/:id/cost/:weight", async (req, res) => {
        ...
        try {
            const totalCost = await cheeseService.calculateCheeseCost(cheeseId, weight);
